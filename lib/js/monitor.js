...
function refreshForm(currentRoom, pwd, tgtTemp, wind, crtTemp, cost) {
	...
	$('div#cost h1.rawInfo').text(cost.toFixed(1));
	$('div#currentTemp h1.rawInfo').text(crtTemp.toFixed(1));
}
...
socket.on('connect', function () {
	...
	socket.on('serviceEvent', function (data) {
		...
		if (data.type === -1) {
			...
			for (let roomNo in roomsInfo) {
				let tmpPos = myDoughnut.data.labels.indexOf(roomNo);
				if (tmpPos === -1) {
					myDoughnut.data.labels.push(roomNo);
					myDoughnut.data.datasets.forEach(function (dataset) {
						dataset.data.push(roomsInfo[roomNo]);
						dataset.backgroundColor.push(pickRandColor());
					});
				} else {
					myDoughnut.data.datasets.forEach(function (dataset) {
						dataset.data[tmpPos] = roomsInfo[roomNo];
					});
				}
			}
			myDoughnut.update();
